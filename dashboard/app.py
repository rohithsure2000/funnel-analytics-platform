...
Two modes, chosen automatically: LIVE if data/processed/warehouse.db
exists (you've run `python -m src.pipeline` against the real data), else
DEMO -- falls back to the precomputed files in dashboard/demo_data/ (see
src/export_demo_data.py), e.g. for a Streamlit Cloud deployment where the
~200MB dataset isn't bundled. A banner shows which mode is active.
